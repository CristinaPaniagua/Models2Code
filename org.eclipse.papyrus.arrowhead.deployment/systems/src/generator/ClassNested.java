...
public class ClassNested {
	 ...
	 * ???
	 * 
	 * @param elements List of elements in payload
	 * @param parentClass The parent class of the payload
	 * @return A dummy list of classes
	 */
	public ArrayList<String> complexelement(ArrayList<String[]> elements, String parentClass) { // TODO Not Used
		boolean boo = false; // TODO Not Used
		for (int i = 0; i < elements.size(); i++) {
			String e = elements.get(i)[0];
			if (e != null) {
				if (e.equals("Newclass")) {
					i = "StopClass".equals(elements.get(i + 1)[0]) ? elements.size() + 1 : genComplex(elements, i, parentClass);
					boo = true;
				}
			}
		}
	...
	public int genComplex(ArrayList<String[]> elements, int i, String parentClass) {
		...
		classesDummy.add(c.dummyobject(className, newclass));
